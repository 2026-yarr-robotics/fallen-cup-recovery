...
# 0.07→0.05 로 줄여 release z 를 2cm 올림(바닥과 맞닿아 충돌 감지되던 문제 회피).
PLACE_Z_DROP     = 0.05
    ...
    def run(self):
        ...
        if self.sim:
            ...
            while rclpy.ok() and time.time() - t0 < SAMPLE_COLLECT_SEC:
                ...
                if len(self.grasp_samples) >= MIN_SAMPLES:
                    break
            # 종료 가드: 이미지에서 mouth-up-cup 이 확실히(>=MIN_SAMPLES) 안 보이면
            # "처리할 컵 없음"(RECOVER_NONE)으로 보고 단계를 정상 종료한다. 0개는
            # 물론, 1..MIN_SAMPLES-1 개의 약한/순간 검출(저신뢰 phantom)도 여기서
            # 걸러 멀쩡한 컵을 건드리지 않는다. (예전엔 1..MIN-1 개가
            # _compute_cup_base 실패 → RECOVER_FAIL 로 빠져 단계가 fail 로 끝나거나
            # phantom 을 컵으로 보고 집을 위험이 있었다.) orchestrator 는
            # RECOVER_NONE 에서 stage 를 break → 노드 종료 → exit 0 으로 이어진다.
            n_samples = len(self.grasp_samples)
            if n_samples < MIN_SAMPLES:
                if n_samples == 0:
                    log.info(
                        "[Sense] /mouth_up_cup/grasp_pose 0개 — 처리할 "
                        "mouth-up-cup 없음 → 단계 정상 종료(RECOVER_NONE)")
                    log.info("  (비전 노드 미동작이어도 동일하게 보이니 노드 상태도 확인)")
                else:
                    log.info(
                        f"[Sense] mouth-up 샘플 부족({n_samples}<{MIN_SAMPLES}) — "
                        "확실한 컵 아님(phantom 가능) → 단계 정상 종료(RECOVER_NONE)")
                return RECOVER_NONE
...
def main(args=None):
    ...
    try:
        ...
        node.destroy_node()
        rclpy.shutdown()
    # 종료코드 정책(outlier orchestrator 와 동일): 복구 완료/처리할 컵 없음 →
    # 0(성공), 감지했으나 처리 실패 → 1. 서버 LaunchManager 가 이 exit code 로
    # task status(idle/failed)를 정하고, 그게 /action_result 로 이어진다.
    sys.exit(0 if status in (RECOVER_DONE, RECOVER_NONE) else 1)

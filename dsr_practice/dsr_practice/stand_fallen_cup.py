...
CUP_R_AT_GRIP = 0.025    # grip 지점 컵 반경. depth(컵 표면) → 컵 축 보정.
                         # grip이 narrow 끝에서 2.5cm 들어간 지점 → 반경 약 2.5cm
                         # (taper 때문에 1.5cm 그립 때(≈2.25cm)보다 ~2.5mm 큼;
                         #  narrow 지름 4.5~5.0cm 가정). 더 가파르게 좁아지면 이 값 키워야.
...
# 인식 안정화
SAMPLE_COLLECT_SEC = 2.5   # grasp_pose 수집 대기 시간 (최대)
...
def main(args=None):
    ...
    try:
        ...
        if ok:
            # 성공이 확정된 뒤에는 cleanup crash 가 결과를 덮어쓰지 않게 한다.
            # node.destroy_node()/rclpy.shutdown() 의 MoveItPy C++ 소멸자가
            # teardown 중 SIGABRT(종료코드 -6) 를 내면 OS exit code 가 -6 으로
            # 덮여, 컵을 이미 세우고 HOME 복귀까지 끝낸 '성공' 을 서버
            # LaunchManager 가 'failed' 로 보고한다. 로그를 flush 한 뒤
            # os._exit(0) 으로 소멸자/atexit 을 통째로 건너뛰어 깨끗이 0 으로
            # 빠진다 (1회성 태스크라 리소스는 OS 가 회수).
            print(
                "[stand_fallen_cup] TASK_RESULT=SUCCESS — recovery 성공 "
                "(종료코드 0, cleanup 우회)",
                file=sys.stderr,
            )
            sys.stderr.flush()
        ...
        node.destroy_node()
        rclpy.shutdown()
    # 실패 경로: 정리 후 종료코드 1 로 서버에 failed 통보. cup_stack wrapper
    # launch 가 이 코드를 전파하고 서버 LaunchManager 가 task 상태로 반영한다.
